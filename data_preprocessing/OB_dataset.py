...
from ..utils.utils import RemoveSignals, OB_service_name_extract_from_pod
...
class OB_dataset:
    ...
    def _get_metric_data(self, metric_path) -> dict:
        ...
        for metric_file in os.listdir(metric_path):
            if "frontend" in metric_file or "service-" in metric_file:
                if "frontend" in metric_file:
                    svc_name = "frontend"
                else:
                    svc_name = metric_file.split("service")[0] + "service"
                metric_pd = pd.read_csv(os.path.join(metric_path, metric_file))
                # print(metric_file, metric_pd.columns)
    ...
    def label_abnormal(self, pd_data: pd.DataFrame, err: str):
        ...
        for i in range(pd_data.shape[0]):
            # print(pd_data.loc[i]['StartTime'], pd_data.loc[i]['PodName'], pd_data.loc[i]['StartTime'] > err_time)
    ...
    def _load_all(self):
        ...
        fault_list_filename = os.path.basename(self.data_path).replace("_OB", "-fault_list.json")
        ...
        if os.path.exists(fault_path):
            ...
            for key in fault_list.keys():
                for i in range(len(fault_list[key])):
                    ...
                    fault_type = OB_service_name_extract_from_pod(fault["inject_pod"])
            ...
            for traceid_idx in tqdm(range(len(all_traceid_file))):
                ...
                # for i in range(0, 100):
                    ...
                    if logs_df.shape[0] > 0:
                        ...
                        loglines = ""
                    new_data = [curr_spanid, traces.loc[i]['PodName'], traces.loc[i]['OperationName'], traces.loc[i]['StartTimeUnixNano'] // 1000000, traces.loc[i]['EndTimeUnixNano'] // 1000000, "", loglines]
            ...
            for err in abnormal_dict.keys():
                for traceid_filename in abnormal_dict[err]:
                    ...
                    # for i in range(0, 100):
                        ...
                        svc_name = OB_service_name_extract_from_pod(traces.loc[i]['PodName'])
                        ...
                        if abs(curr_time_diff) <= 30:
                            ...
                            while True:
                                ...
                                curr_time_diff = start_time - metric_data[svc_name].loc[most_close_metric_idx]['TimeStamp']
                                print(curr_time_diff, svc_name, start_time, metric_data[svc_name].loc[most_close_metric_idx]['TimeStamp'])
                                if abs(curr_time_diff) <= 30:
                                    for metric in neccessary_metrics:
                                        metrics.append(metric_data[svc_name].loc[most_close_metric_idx][metric])
                                    break
